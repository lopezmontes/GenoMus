...
The piece was modeled using the \emph{piano} species which, according to the formalism stated in section~\ref{sec:formaldef}, is defined as a set $G_{piano} = \{P, \vec{e}, T, i, F_{piano}, E, L, M, C, \vec{d}\}$, where
...
\item $\vec{d}$ is a generic data structure as input and output for all functions $\in F_{piano}$.
\end{itemize}


    

There exist many ways to model the piece, since its patterns can be obtained by different methods.\footnote{Comparing different ways of generating this work is an interesting question beyond this paper. Each model can correspond to alternative manners of perceiving structural aspects. A different analysis and procedural model of \emph{Clapping Music} can be found in \cite{MPS-manual}. To create our model, we have looked for the concisest code assembling simple and generic operations.} Our model combines five simple operations using the genotype functions showed in Table~\ref{table:funcs}.
...
\toprule
function name & arguments & output type & description\\
\midrule
\texttt{vMotifLoop} & (\emph{lnotevalueF}, \emph{lmidipitchF}, \emph{larticulationF}, \emph{lintensityF}) & \emph{voiceF} & Creates a sequence of events based on repeating lists. The number of events is determined by the longest list.\\
...
\begin{figure}
  ...
  \label{fig:clapping-struct}
\end{figure}




Our framework has been implemented using JavaScript for the core code, and Max\footnote{\url{https://www.cycling74.com}} and its bach\footnote{\url{https://www.bachproject.net}} package to interact in realtime with the results of the generative processes. Using such interface, the complete work can be produced with the decoded genotype~\ref{lst:clapgeno}.

\subsection{Decoded genotype: a procedural function tree of the piece}
\begin{lstlisting}[float,floatplacement=H, language=Java, caption={Decoded genotype of \emph{Clapping Music} model.}, label={lst:clapgeno}]
...
\end{enumerate}}
\hrulefill
\caption{Subexpressions stored during the evaluation of \emph{Clapping Music} genotype.}
\label{table:subexp}
\end{table}
