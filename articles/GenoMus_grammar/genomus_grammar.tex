...
Our model derivates the whole composition as transformations of the first four notes (motif $A$), appling five generic operations using the genotype functions explained in Table~\ref{table:funcs}. The remaining functions are mere containers for different parameter types.
...
\toprule
function name & argument type & output type & description\\
\midrule
...
In order to... we can visualize the array of floats as a row of lines: the length correspond to values within interval $[0,1]$; colors are mapped to distinguish slight differences among very similar values, so the patterns have a clear visual identity. The encoded phenotype of our \emph{Clapping Music} model consist of 117 values. Figure~\ref{fig:boat3} shows the visualization of this abstract representation of the composition.
