...
Any random sequence of numbers generates a valid program. This sequences are named \emph{germinal vectors}, and will be represented as the set $G_{vec}$:

$$ G_{vec} = \{g_n \in \mathbb{R} \: | \: 0 \leq g_n \leq1 \} $$
...
Let $E_{gen}$ be the set of all possible valid encoded genotypes. To create genotypes, the map $f$ works as a decision tree such as every vector $\vec{g} = (g_1, g_2, ..., g_n) \in G_{vec}^n$ will be mapped to a new vector $\vec{e_g} = (x_1, x_2, ..., x_n) \in E_{gen}$: 
$$f:\ G_{vec}^n\to E_{gen}$$
...
If $\vec{g}$ has more items than needed, they are ignored. If $f$ needs more items than those supplied by $\vec{g}$, $f$ reads $\vec{g}$ repeatedly from the beginning as a loop, until closing the valid encoded genotype. That implies that even vectors with a single value can be mapped to large functional expressions.  
