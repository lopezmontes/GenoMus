...
Although its target is not only genetic algorithms, our framework uses the evolutionary metaphor, similarly as many other automatic composition systems \cite{Sulyok2019, SnchezQuintana2013, add more...}. However, each system has its particular application of the evolutionary recurrent terms, sometimes denoting even opposite concepts. 

In our functional approach, the key idea is \emph{considering a piece of music as the product of a program} which encloses compositional procedures. From this point of view, the definitions of the bio-inspired terms in the context of the \textsl{GenoMus} framework are the following: 


\begin{description}
\item[genotype:] Compositional procedures expressed as a functional tree.
...
\item[decoded genotype:] Genotype coded as a readable and understandable string.
...
\item[specimen:] A genotype/phenotype pair content in a dictionary along with metadata and additional analytical informations.
...
\item[species:] Group of specimens that share the same parameter structure of their musical events.
\end{description}

The \emph{genotype}, in its decoded format, is actually a computable expression. Consequently, the automatic writing and manipulation of genotypes can be seen as a metaprogramming process.

The \emph{phenotype}, as the product of the evaluation of the functional expression content in a genotype, is a sequence of sonic events declared in a format designed to encode music according to a hierarchical structure, made combining these three formal categories:
...
A \emph{species} is characterized by the internal structure of the events inside a score. The species called \emph{piano}, used in the examples, requires four attributes for each event: duration, pitch, articulation and intensity. 

A \emph{parameter} required by a species event can be a single value or an array of values (a \emph{multiparameter}). For instance, in the piano species, an event can contain more than one pitch, to work with chords.
...
A \textsl{GenoMus} framework for a species $s$ is defined as the 10-tuple $G_s = \{P, \vec{e}, T, i, F, E, L, M, C, \vec{d}\}$, where
...
The decoding of \textsl{GenoMus} encoded phenotypes into standard formats for any musical application can be seen as an external operation, not concerning directly this framework. 
