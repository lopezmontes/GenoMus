...
Any random sequence of $n$ numbers $\in [0, 1]$ is a \emph{germinal vector}. A germinal vector determines a decision tree for the core algorithm that leads to an encoded genotype, a valid executable expression that represents musical procedures. The set $G_{vec}^n$ contains every vector  $\vec{g} = (g_1, g_2, ..., g_n)$ of cardinality $n$:

$$ G_{vec}^n = \{\vec{g} \in \mathbb{R}^n \: | \: 0 \leq g_n \leq1 \} $$

Let $G_{vec}$ be the multidimensional search space of all possible of germinal vectors of any cardinality\footnote{Obviously, in a practical implementation, the theoretical infinite germinal vectors are reduced to a finite subset due to computability and memory limits, as well as the number of significant figures used to store and operate on the vectors.}:

$$ G_{vec} = \bigcup_{i=1}^{n}G_{vec}^i \: | \: i \in \mathbb N_{> 0} $$

Each germinal vector in $G_{vec}$ is mapped to a valid encoded genotype (that is a valid functional expression which generates an excerpt of music.
Let $E_{gen}$ be the set of all possible valid encoded genotypes. To create genotypes, the map $f$ works as a decision tree such as every vector $\vec{g} = (g_1, g_2, ..., g_n) \in G_{vec}^n$ will be mapped to a vector $\vec{e_g} = (x_1, x_2, ..., x_n) \in E_{gen}$: 

$$f:\ G_{vec}^n\twoheadrightarrow E_{gen}$$

As the number of choices needed to complete a valid valid encoded genotype $E_{gen}$ rarely match the number of items of a germinal vector $G_{vec}^n$, truncation of loops are employed:
